...
func InitConfig() {
	...
	if err := v.Unmarshal(&global.NocasConfig); err != nil {
		panic(err.Error())
	}

	zap.S().Infof("配置信息 ：%v", global.NocasConfig)
	// 创建clientConfig
	clientConfig := constant.ClientConfig{
		NamespaceId:         global.NocasConfig.Namespace, // 如果需要支持多namespace，我们可以场景多个client,它们有不同的NamespaceId。当namespace是public时，此处填空字符串。
		TimeoutMs:           5000,
	...
	serverConfigs := []constant.ServerConfig{
		{
			IpAddr:      global.NocasConfig.Host,
			ContextPath: "/nacos",
